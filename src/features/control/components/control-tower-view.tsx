...
import type { Alert, StudyPulse } from '../model'

/** Presentación pura: recibe el tablero ya construido y no consulta nada. */
export function ControlTowerView({
  alerts,
  pulses,
}: {
  alerts: readonly Alert[]
  pulses: readonly StudyPulse[]
}) {
  return (
  ...
  return (
    <li>
      <Link
        ...
        className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1 rounded-md border border-l-2 border-border bg-surface px-4 py-3 transition-colors hover:border-accent"
    ...
    <li>
      ...
      >
        ...
        <p className="mt-2 text-sm text-muted">
          {finished ? controlCopy.finished : `${studiesCopy.currentStage}: ${pulse.currentStageName}`}
        </p>
